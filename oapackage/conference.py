...
# import numpy as np
from numpy import ndarray, array, zeros, eye, ones, linalg, log, exp, trace, mean
...
def momentMatrix(k: int) -> ndarray:
    ...
    M[1 : (k + 1), 1 : (k + 1)] = eye(k) * 1.0 / 3
    M[(k + 1) : int(0.5 * k * (k + 1) + 1), (k + 1) : int(0.5 * k * (k + 1) + 1)] = eye(int(0.5 * k * (k - 1))) / 9.0
    M[int(0.5 * k * (k + 1) + 1) :, int(0.5 * k * (k + 1) + 1) :] = eye(k) / 5 + (ones((k, k)) - eye(k)) / 9.0
    return M
...
def _leftDivide(A, B):
    ...
    """
    solution = linalg.solve(A.T.dot(A), A.T.dot(B))
    return solution
...
def modelStatistics(dsd, verbose=0, moment_matrix=None, use_condition_number=True):
    ...
    modelmatrix = array(oapackage.conference_design2modelmatrix(dsd, "q", verbose=0))
    M = (modelmatrix.T).dot(modelmatrix)

    if use_condition_number:
        fullrank = linalg.cond(M) < 1000
    else:
        mr = linalg.matrix_rank(M)
    ...
    if verbose >= 2:
        print("modelStatistics: condition number: %s" % (linalg.cond(M)))
    if fullrank:
        ...
        kappa = linalg.det(M)
        lnkappa = log(kappa) / (pk)
        Defficiency = exp(lnkappa)

        apv = trace(_leftDivide(M, moment_matrix))
        invAPV = 1 / apv
    else:
        Eest = 0
        Defficiency = 0
...
def conferenceProjectionStatistics(array, ncolumns=4, verbose=0):
    ...
    Eestx = zeros(number_combinations)
    Deff = zeros(number_combinations)
    invAPV_values = zeros(number_combinations)
    ...
    pec, pic, ppc = mean(Eestx), mean(Deff), mean(invAPV_values)
    if verbose:
        print(
