...
# import numpy as np
from numpy import array, testing, all, frombuffer, sum, asarray, mean, arange
import oapackage
...
class TestModelmatrix(unittest.TestCase):
    ...
    def test_modelmatrix_main_effects(self):
        ...
        array = oapackage.array_link(array([[0, 1, 2, 3]]).T)

        M = oapackage.array2modelmatrix(array, 'm')
        hc = oapackage.oahelper.helmert_contrasts(4)
        testing.assert_array_almost_equal(hc, M[:, 1:])
    ...
    def test_modelmatrix(self):
        ...
        testing.assert_array_equal(2 * array(al) - 1, model_matrix[:, 1:])
        ...
        last_column = array(conf_design)[:, -1]

        testing.assert_array_equal(model_matrix[:, -1], last_column * last_column)
    ...
    def test_array2eigenModelMatrixMixed(self):
        ...
        main_effects = r[0]
        testing.assert_array_equal(main_effects, array([[-1., -1., -1., -1., 1., 1., 1., 1.]]).T)
        ...
        testing.assert_array_almost_equal(main_effects, array([[-1.22474487, -0.70710678, -1.22474487, -0.70710678],
        ...
        testing.assert_array_almost_equal(interaction_model, array([[1.5, 0.8660254, 0.8660254, 0.5],
...
class TestConferenceDesigns(unittest.TestCase):
    ...
    def test_conf2dsd(self):
        ...
        testing.assert_array_equal(conf, dsd[0:al.n_rows, :])
        testing.assert_array_equal(conf, -dsd[al.n_rows:2 * al.n_rows, :])
        testing.assert_array_equal(0 * conf[0], dsd[-1, :])
    ...
    def test_double_conference_foldover_permutation(self):
        ...
        N = int(al.n_rows / 2)
        folded = array(oapackage.array_link(array(al)[permutation, :]))
        testing.assert_array_equal(folded[0:N, :], -folded[N:, :])
        ...
        al = oapackage.array_link(array(al)[expected, :])
        permutation = oapackage.double_conference_foldover_permutation(al)
        folded = array(oapackage.array_link(array(al)[permutation, :]))
        N = int(al.n_rows / 2)
        testing.assert_array_equal(folded[0:N, :], -folded[N:, :])
...
class TestCppLibrary(unittest.TestCase):
    ...
    def test_get_time_ms(self):
        ...
        testing.assert_almost_equal(dt, 0.2, decimal=1)
    ...
    def test_exampleArray(self):
        ...
        with mock.patch('sys.stdout', new_callable=io.StringIO) as mock_stdout:
            ...
            self.assertTrue(all([lines[ii].startswith('exampleArray %d:' % ii) for ii in range(len(lines))]))
    ...
    def test_distance_distribution_sum(self):
        ...
        testing.assert_array_equal(D, [2.5, 4.5, 7.5, 1.5])
        self.assertEqual(sum(D), 16.)
    ...
    def test_distance_distribution_mixed(self):
        ...
        testing.assert_array_equal(dd0, dd)
        testing.assert_array_equal(dd0, dd_mixed)
    ...
    def test_distance_distribution_mixed_2(self):
        ...
        gt = array([[[ 40.,   0.,   0.,   8.], [ 16.,  16.,  16.,  48.]], [[  0.,  24., 120.,   0.],        [ 16., 176.,  80.,  16.]]])
        self.assertEqual(dd.shape, tuple(d.dims) )
        testing.assert_array_equal(N*dd, gt)
    ...
    def test_projection_efficiencies(self):
        ...
        testing.assert_almost_equal(mean(d), 0.99064112542249538329031111061340197921)

        pec_seq = oapackage.PECsequence(al)
        testing.assert_equal(pec_seq, (1.0,) * len(pec_seq))
        pic_seq = oapackage.PICsequence(al)
        testing.assert_almost_equal(pic_seq, (0.9985780064264659, 0.9965697009006985, 0.9906411254224957,
    ...
    def test_arraylink_slicing(self):
        numpy_array = arange(0, 6 * 10).reshape((6, 10))
        ...
        testing.assert_equal(al[0:4, 1:5], array(al)[0:4, 1:5])
        testing.assert_equal(al[0:1, 0:10:2], array(al)[0:1, 0:10:2])
        testing.assert_equal(al[3, 3::], array(al)[3:4, 3::])
        testing.assert_equal(al[2, :8:2], array(al)[2:3, :8:2])
        testing.assert_equal(al[2:3, :8:2], array(al)[2:3, :8:2])
